...
must_haves:
  ...
  key_links:
    ...
    - from: "Handler"
      to: "MessageProvider"
      via: "container.message.{namespace}.{method}(user_id=user.id, ...)"
...
**Purpose:** Close the critical gap identified in VERIFICATION.md - the session tracking infrastructure (SessionMessageHistory service + enhanced _choose_variant) is fully implemented but not integrated into the actual message generation flow. Providers call _choose_variant without session context parameters, so the session-aware code path never executes.

**Root Cause:** Message providers (user_start.py, admin_vip.py, etc.) call `self._choose_variant(variants, weights)` without the optional `user_id`, `method_name`, `session_history` parameters. Since these default to `None`, the backward-compatible random selection path executes (base.py lines 139-146), and the session-aware code path (lines 148-181) never runs.
...
- All message provider methods accept optional `user_id` parameter
...
gap:
  ...
  status: partial
  reason: "SessionMessageHistory service exists with full implementation and BaseMessageProvider._choose_variant accepts session context, but message providers (user_start.py, admin_vip.py, etc.) do NOT pass session context when calling _choose_variant. The infrastructure exists but is not wired into the actual message generation flow."
  missing:
...
5. **Backward Compatibility:**
   ...
   print('Backward compat:', '🎩' in text and 'Juan' in text)
   "
   ```
...
2. Summary of changes (ServiceContainer integration, provider modifications)
3. Files modified: container.py, __init__.py, 6 provider files
